...
def blast_n_stuff(strain, our_fasta_loc):
    ...
    for r in SeqIO.parse("PaVE.gb","genbank"):
        if r.id.split(".")[0]==ref_seq_gb:
            e = open(strain + SLASH + strain + '_ref.gbk', 'w')
            SeqIO.write(r, e, "genbank")
            e.close()
...
def pull_correct_annotations(strain, our_seq, ref_seq, genome):
    ...
    #print gene_loc_list, gene_product_list, all_loc_list, all_product_list, name_of_the_feature_list  
